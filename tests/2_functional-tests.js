  ...
  suite("10 functional tests", function(){
    test("Creating a new thread: POST request to /api/threads/{board}", function(done){
      ...
      .end(function(err, res) {
        ...
        assert.equal(res.body.threads[0].text, "test text");
        assert.equal(res.body.threads[0].delete_password, "test");
        assert.equal(res.body.threads[0].reported, false)
        testThread_id = res.body.threads[0]._id;
        board = res.body.name;
    ...
    test("Deleting a thread with incorrect password: DELETE request to /api/threads/{board} with an invalid delete_password", function(done){
      chai
      .request(server)
    ...
    test("Reporting a thread: PUT request to /api/threads/{board}", function(done){
     ...
     //console.log("board", board);
      ...
      .set("content-type", "application/json")
      .send({ thread_id: testThread_id, name: board })
      .end(function(err, res) {
        assert.equal(res.status, 200);
    ...
    test("Deleting a reply with the incorrect password: DELETE request to /api/replies/{board} with invalid delete_password", function(done){
      console.log(
        "delete reply invalid ids: thread_id:" + testThread_id + " reply_id" + testReply_id
      );
      chai
      .request(server)
